...
def five_of_a_kind(hand):
    ...
    if count.iloc[0] == 5:
        ...
        if 'J' in hand:
            hand_tmp = hand.replace('J','*')
            keys = count.keys()
            for key in keys:
                if key != 'J':
                    # Replace the highest amount..
                    hand_tmp = hand_tmp.replace(key, '*')
                    return five_of_a_kind(hand_tmp)
            pass
        return False
        ...
        if 'J' in hand:
            ...
            for key in keys:
                if key != 'J':
                    # Replace the highest amount..
                    hand_tmp = hand_tmp.replace(key, '*')
                    return four_of_a_kind(hand_tmp)
            pass
        return False
        ...
        if 'J' in hand:
            ...
            for key in keys:
                if key != 'J':
                    # Replace the highest amount..
                    hand_tmp = hand_tmp.replace(key, '*')
                    return three_of_a_kind(hand_tmp)
            pass
        return False
